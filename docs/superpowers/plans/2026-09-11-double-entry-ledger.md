...
**Invariant trigger:** a Postgres function + `CONSTRAINT TRIGGER ... DEFERRABLE INITIALLY DEFERRED ... FOR EACH ROW` that, at commit, checks the `transaction_id` of the affected row(s) (branch on `TG_OP`: use `NEW.transaction_id` for INSERT/UPDATE, `OLD.transaction_id` for UPDATE/DELETE, so an entry moved between transactions checks both sides): `SUM(debit amounts) - SUM(credit amounts) = 0`, raising an exception otherwise. **Correction from the originally planned design:** PostgreSQL's `CREATE CONSTRAINT TRIGGER` does not support `FOR EACH STATEMENT` or `REFERENCING NEW TABLE` at all — constraint triggers in Postgres can only be `FOR EACH ROW` (confirmed against a live Postgres 16 instance during implementation). This does not weaken the guarantee: because the trigger is deferred, its function's execution is delayed until COMMIT regardless of row-vs-statement granularity, so by the time any row's trigger fires, every sibling row from the same (possibly multi-row) INSERT is already persisted and visible to a fresh query against `entries` — the function must re-query `entries` directly rather than relying on `NEW`/a transition table, since no transition table exists at row granularity. The function must also set `SET search_path = pg_catalog, public, pg_temp` (with `pg_temp` explicitly listed, since Postgres implicitly searches the temp schema first for unqualified relation names regardless of `search_path` contents unless `pg_temp` is explicitly given a position in it) and schema-qualify the query as `FROM public.entries`, otherwise a session with a temp table named `entries` silently bypasses the check.

- [ ] Write the three migrations (transactions table, entries table, trigger function + constraint trigger). Run `npm run migrate`.
- [ ] Write a failing test proving the trigger is load-bearing: inside a raw Knex transaction, insert one unbalanced pair of entries (e.g. debit 100 / credit 50) under a fresh transaction row and assert the commit throws with an "unbalanced" message; insert a balanced pair and assert it commits and both rows land. Also test the temp-table shadowing bypass directly: inside a transaction, `CREATE TEMP TABLE entries (...)`, insert an unbalanced pair into `public.entries`, and confirm the commit is still rejected.
- [ ] Run migrations/tests, fix the trigger SQL if the unbalanced case doesn't throw.
- [ ] Add a code comment near the trigger warning that application code must never issue `SET CONSTRAINTS ALL IMMEDIATE` in a session doing multi-leg inserts — it would force the trigger to fire per-row mid-statement and reject a legitimate balanced insert on its first row (fails closed, not a soundness bug, but worth documenting).
- [ ] Commit.
